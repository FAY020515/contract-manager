...
export const api = {
  ...
  getPaymentsByContract: (contractId: number) =>
    request<any[]>(`/contracts/${contractId}/payments`),

  getPaymentSummary: (contractId: number) =>
    request<any>(`/contracts/${contractId}/payments/summary`),

  createPayment: (contractId: number, data: any) =>
    request<{ id: number }>(`/contracts/${contractId}/payments`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  uploadPaymentAttachment: (id: number, file: File) => {
    const formData = new FormData();
    formData.append('file', file);
    return request<{ filename: string; original_name: string }>(`/payments/${id}/attachment`, {
      method: 'POST',
      body: formData,
    });
  },

  deletePaymentAttachment: (id: number) =>
    request<{ success: boolean }>(`/payments/${id}/attachment`, {
      method: 'DELETE',
    }),
