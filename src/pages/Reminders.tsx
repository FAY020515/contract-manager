...
import {
  BellOutlined, EditOutlined, CalendarOutlined,
...
const Reminders: React.FC = () => {
  ...
  const handleProcess = async (record: Reminder) => {
    // 如果提醒已完成，先重置为待处理
    if (record.status === 'processed') {
      try {
        await window.api.updateReminderStatus(record.id, 'pending');
        setReminders(prev => prev.map(r =>
          r.id === record.id ? { ...r, status: 'pending' } : r
        ));
      } catch (e) {
        console.error(e);
      }
    }
    navigate(`/contracts/${record.contract_id}/edit`);
  };
